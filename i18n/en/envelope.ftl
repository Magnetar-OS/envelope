...
no-accounts-description = No accounts yet. Add one below. An account added in Slate or Circle appears here too, with its password — it only needs a mail server.
no-mail-server = No mail server set
set-up-mail = Set up
add-account = Add account…
add-account-title = New account
add-account-name = Your name
add-account-address = Email address
password = Password
app-password-hint = Many providers want an app-specific password rather than your normal one.
provider-uses-sign-in = { $provider } accounts sign in with the browser. Use the button below.
provider-sign-in-unavailable = { $provider } normally signs in with the browser, which is not set up on this computer. An app password over IMAP may still work.
add = Add
adding = Finding your mail server…
add-needs-address = Type a whole email address.
add-needs-password = Type the password.
add-no-server = The account is saved, but its mail server could not be found: { $reason }. Fill it in here.
change = Change
